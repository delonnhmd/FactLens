// PHASE 4 STEP 11
export type ClaimDraftQualityLevel = "good" | "warning" | "bad";
...
export function analyzeClaimDraft({ title, description = "", sourceUrl = "", category = "" }: ClaimDraftInput): ClaimDraftAnalysis {
  ...
  if (!normalizedTitle) {
    warnings.unshift("Add a clear factual claim before posting.");
    ...
    warnings.unshift("This reads like a question instead of a factual claim.");
    ...
    warnings.unshift("This claim is too vague to fact-check reliably.");
    suggestions.push("Add who, what, when, and source.");
  } else if (isPromotion(combinedText)) {
    detectedType = "PROMOTION";
    warnings.unshift("This reads like promotion instead of a neutral claim.");
    ...
    warnings.unshift("This reads like personal opinion and may not be verifiable.");
    ...
    warnings.unshift("This claim may need more factual detail before the community can verify it.");
    suggestions.push("Add who, what, when, and source.");
  }

  const okToSubmit = detectedType === "FACTUAL";
  const qualityLevel: ClaimDraftQualityLevel =
    detectedType !== "FACTUAL" ? "bad" : warnings.length > 0 ? "warning" : "good";
