...
export default async function MyClaimsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-[680px]">
      ...
      {claims.length ? <section className="space-y-5" aria-label="Your claims">{claims.map((claim) => <ClaimCard claim={claim} key={claim.id} />)}</section> : <EmptyState title="No claims yet" description="Create your first source-backed claim to begin participating." />}
    </div>
  );
}
